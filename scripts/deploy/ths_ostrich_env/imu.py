...
class IMUReader:
    ...
    通过 imu_type 选择 IMU 型号：
      ...
      - "yis320":    YESENSE YIS320（被动上报，帧头 0x59 0x53，变长 TLV 帧，460800 波特率）
    """
    ...
    IMU_DEFAULTS: ClassVar[dict[str, dict[str, int | bytes]]] = {
        ...
        "yis320": {
            "baud_rate": 460800,
            "frame_head": b"\x59\x53",
            "frame_min_len": 7,  # 变长 TLV 帧，此处仅为最小帧长
        },
    }
    ...
    @staticmethod
    def _parse_yis320_frame(frame):
        """解析 YIS320 帧（帧头 0x59 0x53，变长 TLV 载荷），返回 (gyro, euler, quat)，单位：度

        TLV 数据项：0x20 角速度、0x40 欧拉角（原始顺序 pitch, roll, yaw）、0x41 四元数。
        """
        gyro = None
        euler = None
        quat = None
        payload = frame[5:-2]  # 去掉帧头、TID、载荷长度与校验
        offset = 0
        while offset + 2 <= len(payload):
            data_id = payload[offset]
            data_len = payload[offset + 1]
            raw = payload[offset + 2 : offset + 2 + data_len]
            offset += 2 + data_len
            if data_id == 0x20:
                Wx, Wy, Wz = struct.unpack("<iii", raw)
                gyro = (Wx * 0.000001, Wy * 0.000001, Wz * 0.000001)
            elif data_id == 0x40:
                pitch, roll, yaw = struct.unpack("<iii", raw)  # YIS320 原始顺序：pitch, roll, yaw
                euler = (roll * 0.000001, pitch * 0.000001, yaw * 0.000001)
            elif data_id == 0x41:
                QW0, QX1, QY2, QZ3 = struct.unpack("<iiii", raw)
                quat = (QW0 * 0.000001, QX1 * 0.000001, QY2 * 0.000001, QZ3 * 0.000001)
        if gyro is None or euler is None or quat is None:
            raise ValueError("YIS320 帧缺少角速度/欧拉角/四元数")
        return gyro, euler, quat
    ...
    def _read_loop(
        ...
        elif imu_type == "yis320":
            cls._yis320_imu_loop(ser, running, imu_data)
        else:
    ...
    @classmethod
    def _yis320_imu_loop(cls, ser, running, imu_data):
        """YIS320 读取循环：帧头 0x59 0x53，长度在第 5 字节，变长 TLV 帧"""
        buffer = b""
        while running.value:
            count = ser.inWaiting()
            if count > 0:
            ...
            head_pos = buffer.find(b"\x59\x53")
            if head_pos == -1:
                buffer = buffer[-1:]  # 可能只收到半个帧头
                time.sleep(0.0001)
                continue
            if head_pos > 0:
                buffer = buffer[head_pos:]

            if len(buffer) < 7:  # 帧头(2) + TID(2) + 长度(1) + 校验(2)
                time.sleep(0.0001)
                continue

            frame_len = 7 + buffer[4]  # 变长帧总长
            ...
            ck1 = 0
            ck2 = 0
            for value in frame[2:-2]:  # 两字节 Fletcher 校验和 (CK1, CK2)
                ck1 = (ck1 + value) & 0xFF
                ck2 = (ck2 + ck1) & 0xFF
            if frame[-2:] != bytes((ck1, ck2)):
                buffer = frame[1:] + buffer  # 校验失败，前移一字节重新同步
                continue
            try:
                gyro, euler, quat = cls._parse_yis320_frame(frame)
