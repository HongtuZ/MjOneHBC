...
class MotionCommand(CommandTerm):
    ...
    def __init__(self, cfg: MotionCommandCfg, env: ManagerBasedRlEnv):
        ...
        self.motion_loader = MotionLoader(
            motion_data_dir=str(Path(self.cfg.motion_data_dir).resolve()),
            motion_data_weights=None,
            device=self.device,
        )
        self.motion = self.motion_loader.get_all_motions(dt=env.step_dt, joint_names=self.robot.joint_names, body_names=self.cfg.body_names)
    ...
    def _update_command(self):
        self.time_steps += 1
        out_of_bounds = self.time_steps >= self.motion.num_frames
        # Only index motion_ids for envs that are still within bounds to avoid
        motion_changed = torch.zeros_like(out_of_bounds)
        valid = ~out_of_bounds & (self.time_steps >= 1)
        if valid.any():
            motion_changed[valid] = (
                self.motion.motion_ids[self.time_steps[valid]]
                != self.motion.motion_ids[self.time_steps[valid] - 1]
            )
        env_ids = torch.where(out_of_bounds | motion_changed)[0]
        if env_ids.numel() > 0:
