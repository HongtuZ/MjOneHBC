...
class MotionCommand(CommandTerm):
    ...
    def __init__(self, cfg: MotionCommandCfg, env: ManagerBasedRlEnv):
        ...
        motion_file_path = Path(self.cfg.motion_file).resolve()
        self.motion_loader = MotionLoader(
            motion_data_dir=str(motion_file_path.parent),
            motion_data_weights={motion_file_path.stem: 1.0},
            device=self.device,
        )
        self.motion = self.motion_loader.get_one_motion(
            motion_id=0, dt=env.step_dt, joint_names=self.robot.joint_names, body_names=self.cfg.body_names
        )
