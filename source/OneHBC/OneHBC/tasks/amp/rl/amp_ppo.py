...
class AmpPPO:
    ...
    def process_env_step(
        ...
        style_reward, _ = self.discriminator.predict_style_reward(self.transition.observations)
        self.transition.rewards = self.discriminator.lerp_reward(style_reward, rewards.clone())
        ...
        self.discriminator.reset(dones)
    ...
    def update(self) -> dict[str, float]:
        ...
        mean_entropy = 0
        mean_amp_loss = 0
        mean_amp_policy_pred = 0
        mean_amp_expert_pred = 0
        mean_amp_grad_penalty_loss = 0
        # RND loss
        ...
        mean_amp_grad_penalty_loss /= num_updates
        ...
        loss_dict = {
            ...
            "amp": mean_amp_loss,
            "amp_policy_pred": mean_amp_policy_pred,
            "amp_expert_pred": mean_amp_expert_pred,
            "amp_grad_penalty": mean_amp_grad_penalty_loss,
        }
        if self.rnd:
    ...
    def construct_algorithm(obs: TensorDict, env: VecEnv, cfg: dict, device: str) -> AmpPPO:
        ...
        default_sets = ["actor", "critic", "discriminator", "discriminator_expert"]
        ...
        cfg["discriminator"].pop("class_name")
        discriminator: AmpDiscriminator = AmpDiscriminator(
            obs, cfg["obs_groups"], "discriminator", 1, **cfg["discriminator"]
        ).to(device)
        ...
        alg: AmpPPO = AmpPPO(
            actor, critic, discriminator, storage, device=device, **cfg["algorithm"], multi_gpu_cfg=cfg["multi_gpu"]
        )
